...
public class Shop : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        _itemsCulture.Add(new Item("Wheat", 5));

        _itemsActionneur = new List<Item>();
        _itemsActionneur.Add(new Item("Alerte", 35));
        _itemsActionneur.Add(new Item("Arroseur", 60));

        _itemsCapteur = new List<Item>();
        _itemsCapteur.Add(new Item("Humidité", 10));
    }
    ...
    void Start()
    {
        ...
        for (int i = 0; i < _itemsCulture.Count; i++)
        {
            ...
            item.GetComponent<ShopItem>().UpdateText(_itemsCulture[i].name, _itemsCulture[i].price);
        ...
        for (int i = 0; i < _itemsActionneur.Count; i++)
        {
            ...
            item.GetComponent<ShopItem>().UpdateText(_itemsActionneur[i].name, _itemsActionneur[i].price);
        ...
        for (int i = 0; i < _itemsCapteur.Count; i++)
        {
            ...
            item.GetComponent<ShopItem>().UpdateText(_itemsCapteur[i].name, _itemsCapteur[i].price);
